 ...
 *   */
const std::string samplesBasePath = "/pnfs/knu.ac.kr/data/cms/store/user/jelee/WtoMuNu_pythia/";
/*
...
std::map<std::string, float> eventWeights = {
     {"100to200",   175.1    / 1426696.0 * integratedLuminosity}, 
     {"200to500",   7.423    / 1437168.0 * integratedLuminosity}, 
     {"500to1000",  0.254    / 1462209.0 * integratedLuminosity}, 
     {"1000to2000", 0.01586  / 1460256.0 * integratedLuminosity}, 
     {"2000to3000", 4.68E-04 / 1415130.0 * integratedLuminosity}, 
     {"3000to4000", 3.20E-05 / 1502968.0 * integratedLuminosity}, 
     {"4000to5000", 2.95E-06 / 1448900.0 * integratedLuminosity}, 
     {"5000to6000", 4.03E-07 / 1411549.0 * integratedLuminosity}, 
     {"6000"      , 1.05E-07 / 1493439.0 * integratedLuminosity}, 
};
/*
...
using ints = RVec<int>;
  auto LEPorigins = [](int nGenPart, ints GenPart_pdgId, ints GenPart_status, floats GenPart_pt)
    ...
    return df.Define("LEPorigins", LEPorigins, 
             ...
             .Filter("mu_idx != -1", "Valid mu in selected idx")
...
int main() {
    ...
    for (const auto &sample : sampleNames) {
        ...
        dfFinal.Snapshot("Events", sample + "_Skim_m_pythia.root", finalVariables);
        time.Stop();
